...
- **Pass 0 (MAP)**: Primary search phase. Identifies mandatory paths and extracts anchor candidates (**Prefix**, **Pivot**, or **Suffix**).
    ...
    - **Variable-Distance Snapping (WarpBack)**: For patterns where an anchor is preceded by a variable repetition, the engine MUST use **WarpBack** (reverse SWAR scan) to find the true leftmost match start before activating the DFA. This ensures high-speed scanning for `.*LITERAL` patterns.
