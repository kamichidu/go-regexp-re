...
- **Pass 0 (MAP)**: Primary search phase. Identifies mandatory paths and extracts anchor candidates (**Prefix**, **Pivot**, or **Suffix**).
    - **Search**: Identifies raw candidate positions using SIMD/SWAR. **Leverages LCP (Longest Common Prefix) Factoring to extract mandatory prefixes and Suffix-based Pivot discovery for patterns with variable-length prefixes (e.g., '@example.com' from '.*@example\.com').**
    - **Variable-Distance Snapping (WarpBack)**: For patterns where an anchor is preceded by a variable repetition, the engine MUST use **WarpBack** (reverse SWAR scan) to find the true leftmost match start before activating the DFA. **Correctness Mandate: Validation of variable-length constraints MUST NOT incorrectly reject candidates due to newlines in the preceding content; such verification is deferred to the Snap phase to ensure high-speed scanning for `.*LITERAL` patterns remains robust across multi-line inputs.**
