...
- **Pass 1: Naked Discovery**: A high-speed scan (DFA or BP-DFA) determines match boundaries `[start, end]` and records the final winning priority without history recording.
- **Pass 2: Anchored Recording**: A precise forward scan re-runs the DFA only over the identified match range `[start, end]` using a clean `MatchState` to generate a noise-free execution history.
...
Path selection in Pass 3 MUST NOT employ runtime NFA simulation or dynamic priority comparisons. The identity of the winning path must be reconstructed solely by following pre-calculated priority transitions (`InputPriority` -> `NextPriority`) stored within the `RecapTable`.
