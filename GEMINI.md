...
- **Two-Phase Warping & Inverted Logic**: The engine MUST distinguish between **SearchWarp** (finding match start) and **CCWarp** (continuing match). SearchWarp MUST skip noise (characters NOT in the start set) using **Inverted Kernel Logic**. To achieve 10 GB/s+ throughput, SearchWarp MUST prioritize SIMD-accelerated library functions:
    - **Single Byte**: Use `bytes.IndexByte`.
    - **ASCII Set**: Use `bytes.IndexAny` with an ASCII-only search string.
    - **Non-ASCII Set**: Use manual `IndexByte` loops to find the earliest lead-byte candidate, avoiding `bytes.IndexAny`'s UTF-8 interpretation of raw bytes.
    - **Fallthrough**: Fall back to custom SWAR kernels for large or negated sets.
