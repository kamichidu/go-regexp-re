...
type BitParallelDFA struct {
	...
	SuccessorTable [8][256]uint64 // Pre-calculated transitions for each 8-bit chunk of the state
	ContextMasks   [64]uint64     // Valid states for each of the 64 possible EmptyOp contexts
	StartMasks     [64]uint64     // Initial states for each starting context
	MatchMasks     [64]uint64     // States that reach InstMatch in each context
	MatchesEmpty   [64]bool       // Whether the pattern matches empty string in each context
}
...
func NewBitParallelDFA(prog *syntax.Prog) *BitParallelDFA {
	...
	// 1. CharMasks: Bits matching each byte
	...
	// 2. Epsilon Closure with Anchor Support
	var epsilonClosure func(int, uint64, syntax.EmptyOp) (uint64, bool)
	epsilonClosure = func(id int, visited uint64, ctx syntax.EmptyOp) (uint64, bool) {
		if (visited & (1 << uint(id))) != 0 {
			return 0, false
		}
		visited |= (1 << uint(id))
		...
		switch inst.Op {
		case syntax.InstAlt, syntax.InstAltMatch:
			m1, a1 := epsilonClosure(int(inst.Out), visited, ctx)
			m2, a2 := epsilonClosure(int(inst.Arg), visited, ctx)
			return m1 | m2, a1 || a2
		case syntax.InstCapture, syntax.InstNop:
			return epsilonClosure(int(inst.Out), visited, ctx)
		case syntax.InstEmptyWidth:
	...
	// 3. ContextMasks, StartMasks & MatchMasks: Resolve anchors for all 64 contexts
	for ctx := 0; ctx < 64; ctx++ {
		emptyOp := syntax.EmptyOp(ctx)
		for i := 0; i < len(prog.Inst); i++ {
			if i >= 64 {
				break
			}
			mask, isAcc := epsilonClosure(i, 0, emptyOp)
			if mask != 0 || isAcc {
				bp.ContextMasks[ctx] |= (1 << uint(i))
			}
			if isAcc {
				bp.MatchMasks[ctx] |= (1 << uint(i))
			}
		}
		start, isAcc := epsilonClosure(prog.Start, 0, emptyOp)
		bp.StartMasks[ctx] = start
		bp.MatchesEmpty[ctx] = isAcc
	}

	// 4. SuccessorTable & MatchMasks: Pre-calculate transition targets
	// successorMasks[i] is the epsilon closure (ignoring anchors) from instruction i.
	successorMasks := [64]uint64{}
	for i := 0; i < len(prog.Inst); i++ {
		if i >= 64 {
			break
		}
		...
		switch inst.Op {
		case syntax.InstRune, syntax.InstRune1, syntax.InstRuneAny, syntax.InstRuneAnyNotNL:
			// For successor table, we want to know what letter-states are reachable
			// after this one, assuming all anchors could potentially be satisfied.
			next, _ := epsilonClosure(int(inst.Out), 0, 63)
			successorMasks[i] = next
		}
	}
	...
	for ctx := 0; ctx < 64; ctx++ {
		...
		for i := 0; i < len(prog.Inst); i++ {
			...
			// A bit is a match in this context if its epsilon closure reaches InstMatch.
			// Crucially, for a letter-matching instruction i, we check if its OUT
			// transition reaches InstMatch given the context.
			inst := prog.Inst[i]
			switch inst.Op {
			case syntax.InstRune, syntax.InstRune1, syntax.InstRuneAny, syntax.InstRuneAnyNotNL:
				_, isAcc := epsilonClosure(int(inst.Out), 0, emptyOp)
				if isAcc {
					bp.MatchMasks[ctx] |= (1 << uint(i))
				}
			}
	...
	for chunk := 0; chunk < 8; chunk++ {
		for val := 0; val < 256; val++ {
			...
			for bit := 0; bit < 8; bit++ {
				if (val & (1 << uint(bit))) != 0 {
					combined |= successorMasks[chunk*8+bit]
				}
			}
			bp.SuccessorTable[chunk][val] = combined
		}
	}
