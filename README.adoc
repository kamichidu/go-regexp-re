...
The engine achieves high throughput by offloading computation to the compilation phase and utilizing bit-parallel execution paths. This model is optimized for long-lived regex objects where the construction cost is amortized over millions of execution cycles.

== Execution Strategies

The engine dynamically selects the most efficient path based on pattern characteristics:

1. **Literal Bypass (Fast Path)**: For pure constant strings and anchored literals. Bypasses all DFA construction and uses direct memory comparison.
2. **DFA Execution (Deterministic Path)**: Converts patterns into a high-density deterministic automaton for guaranteed $O(n)$ matching.
3. **Bit-parallel/SWAR Acceleration (Hybrid Path)**: Accelerates character class scanning and repetitions using bitwise logic expressed in Go (SWAR), reaching physical memory bandwidth limits.

Captured metadata is extracted via a **3-Pass Sparse TDFA** pipeline, ensuring that submatch extraction remains strictly $O(n)$ relative to the input length.
